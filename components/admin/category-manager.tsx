...
import { FolderTree, Pencil, Power, SearchX } from "lucide-react";
...
const CREATE_CATEGORY_DEFAULTS: CreateValues = {
  name: "",
  slug: undefined,
...
function categoryValues(category: FlatCategory): UpdateValues {
  return {
    name: category.name,
    slug: category.slug,
...
function CreateCategoryDialog({ categories }: { categories: FlatCategory[] }) {
  ...
  const { isOpen } = useUrlDialog("category-create");
  const {
    control,
    handleSubmit,
    reset,
    ...
    defaultValues: CREATE_CATEGORY_DEFAULTS,
  });
  useEffect(() => {
    if (!isOpen) reset(CREATE_CATEGORY_DEFAULTS);
  }, [isOpen, reset]);
  ...
  return (
    ...
    >
      {(close) => {
        const closeAndReset = () => {
          reset(CREATE_CATEGORY_DEFAULTS);
          close();
        };
        return (
          <form
...
function EditCategoryDialog({
  ...
  const router = useRouter();
  const { isOpen } = useUrlDialog("category-edit");
  const {
    control,
    handleSubmit,
    reset,
    ...
    mode: "onChange",
    reValidateMode: "onChange",
    defaultValues: category ? categoryValues(category) : {},
  });
  useEffect(() => {
    if (category) reset(categoryValues(category));
  ...
  return (
    <div className="flex flex-1 flex-col gap-4">
      ...
      {visible.length ? (
        ...
        <AdminEmptyState
          icon={search ? SearchX : FolderTree}
          title={search ? "No matching categories" : "No categories yet"}
          description={
            search
              ? "Try another name or slug, or clear the search to see every category."
              : "Create the first category to start organizing products for the storefront."
          }
          action={
            search ? (
              <Button
